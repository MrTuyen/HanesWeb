...
var dashboard = {
    ...
    portLocations:async function(req, res,) {
        var options =req.body.options;
        if(options==1){
            let result = await db.QueryAsync('SELECT Zone FROM Zone GROUP BY zone ORDER BY zone;');
            return res.end(JSON.stringify({ rs: true, msg: "Thành công", data1: result }));
        }
        else if(options==2){
    ...
    postDataSubmit:async function(req, res,){
        ...
        return res.end(JSON.stringify({ rs: true,
            msg: "Thành công", 
            data: {'lossTime':lossTime,'lableRunTime':lableRunTime,'powerOnTime':powerOnTime,'runTime':runTime},
            dataRatio:{'lableRunTimeRatio':lableRunTimeRatio,'ratioLossTime':ratioLossTime,'ratioNoLoad':ratioNoLoad,'ratioRunTime':ratioRunTime}
            }));
    },
    portDownloadReport: async function(req, res, next) {
        var zone = req.body.zone;
        var line = req.body.line;
        var shift = req.body.shift;
        var from_date = req.body.from_date;
        ...
        return res.end(JSON.stringify({ rs: true,
            msg: "Thành công", 
           data:{"dataLossTime":dataLossTime[0],"dataRunTime":dataRunTime[0]}
            }));
    },
    getdetailDashboard: function(req, res, next){
        res.render('Innovation/sewingRealtime/detailDashboard');
    },
    postDetailDataSubmit: async function(req, res,next){
        var zone = req.body.zone;
        var line = req.body.line;
        var shift = req.body.shift;
        var from_date = req.body.from_date;
        ...
        return res.end(JSON.stringify({ rs: true,
            msg: "Thành công", 
            data: {'waitingTime':waitingTime,'repairingTime':repairingTime,'lableRunTime':lableRunTime,'powerOnTime':powerOnTime,'runTime':runTime, 'lineName':lineName,'lableColors':lableColors},
            dataRatio:{'lableRunTimeRatio':lableRunTimeRatio,'ratioWaitingTime':ratioWaitingTime,'ratioRepairingTime':ratioRepairingTime,'ratioRunTime':ratioRunTime,'ratioNoLoad':ratioNoLoad}
            }));
    }
};
