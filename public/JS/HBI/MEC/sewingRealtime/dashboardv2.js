$(document).ready(function () {
    $.ajax({
        url: '/innovation/realtime/locations',
        type: 'post',
        data: {
            'options': 1,
            'b': 32
...
function load() {
    ...
    $.ajax({
        ...
        chartGeneral.data.labels = response.data.lableRunTime;
        chartGeneral.data.datasets[0].data = response.data.powerOnTime;
        chartGeneral.data.datasets[1].data = response.data.runTime;
        chartGeneral.data.datasets[2].data = response.data.lossTime;
        chartRatio.data.labels = response.dataRatio.lableRunTimeRatio;
        chartRatio.data.datasets[0].data = response.dataRatio.ratioRunTime;
        chartRatio.data.datasets[1].data = response.dataRatio.ratioLossTime;
        chartRatio.data.datasets[2].data = response.dataRatio.ratioNoLoad;
        chartGeneral.update();
        chartRatio.update();
        chartOEE.update();
    });
...
function load2(){
    ...
    $.ajax({
        ...
        chartDetailLine1.data.labels = response.data.lableRunTime;
        chartDetailLine1.data.datasets[0].data = response.data.powerOnTime;
        chartDetailLine1.data.datasets[1].data = response.data.runTime;
        chartDetailLine1.data.datasets[2].data = response.data.repairingTime;
        chartDetailLine1.data.datasets[3].data = response.data.waitingTime;
        chartDetailLine1.options.scales.xAxes.ticks.color=response.data.lableColors;

        chartDetailLine2.data.labels = response.dataRatio.lableRunTimeRatio;
        chartDetailLine2.data.datasets[0].data = response.dataRatio.ratioRunTime;
        chartDetailLine2.data.datasets[1].data = response.dataRatio.ratioRepairingTime;
        chartDetailLine2.data.datasets[2].data = response.dataRatio.ratioWaitingTime;
        chartDetailLine2.data.datasets[3].data = response.dataRatio.ratioNoLoad;
        chartDetailLine2.options.scales.xAxes.ticks.color=response.data.lableColors;
        chartDetailLine1.update();
        chartDetailLine2.update();
        $('.line_name').html(`Line: ${zone}${line}`);
        $('.style').html(`Style: ${response.data.lineName[0]['line_name']}`);
    });
}
...
function download_report() {
    ...
    var to_date = document.getElementById("timeCheckOut").value;
    $.ajax({
        url: "/innovation/realtime/downloadReport",
        type: "post",
        data: {
            'zone': zone,
            'line': line,
