...
function Adjusting() {
    ...
    if (oldID != '' && newID != '') {
        $.ajax({
            url: "/innovation/realtime/updateMachine",
            type: "post",
            data: {
                'oldID': oldID,
                'newID': newID
...
function updateLayoutLine(lineName) {
    $.ajax({
        url: '/innovation/realtime/machine',
        type: 'post',
        data: {
            'lineName': lineName
        },
        ...
        for (let row = 0; row < 2; row++) {
            ...
            for (let col = 0; col < 13; col++) {
                ...
                `;
                    ...
                    html +=
                        `<div class="empty col" data-bs-toggle="modal" data-bs-target="#modal-machine" data-bookid="${null}">
...
myModal.addEventListener('show.bs.modal', function () {
    ...
    if (locationLate != null) {
        $.ajax({
            url: '/innovation/realtime/machineData',
            type: 'post',
            data: {
                'lineName': lineName,
                'index': locationLate
            ...
            console.log(response.machineData[0]['operation']);
