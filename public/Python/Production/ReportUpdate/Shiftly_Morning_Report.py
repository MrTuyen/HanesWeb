...
while row<imageFile_len:
    ...
    for i in range(0, len(group_data)):
        ...
        TIME=(group_data.iloc[i,5])
        if QC=='' and TIME<'1320':#1320
            no_QC=no_QC+1
        if SUM_FULL==0 and TIME<'1320':
            no_ID=no_ID+1
        if MODIFIED_USER==2:# and MODIFIED_USER<SUM_BUNDLE:
            modified=modified+1
        if QC=='' and TIME>='1320' and TIME<'1415':#1320, 1415
            wip=wip+1
        done=done+1
...
writer = pd.ExcelWriter(bundleError_link+thisWeek+'\\KickOut_'+date+'_Morning.xlsx', engine='xlsxwriter')
